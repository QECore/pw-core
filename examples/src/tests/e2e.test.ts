...
scenario(
  'End-to-End User Flow on QECore App with Page Object Flows',
  async ({ loginPage, dashboardPage, projectsPage, tasksPage, sidebar, topNav }) => {
    ...
    expect(taskHeaders).toContain('sortProjectsTitle')
    const taskRows = await taskTable.get()

    expect(taskRows.getAll('sortProjectsTitle')).toContain('Demo Task')
